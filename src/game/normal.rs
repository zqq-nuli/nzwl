...
use crate::keys::{
    left_click_legacy, move_down, move_left, move_to, press_key, press_key_sequence,
    tap_key, KeyAction, VK_4, VK_5, VK_A, VK_D, VK_G, VK_O, VK_S, VK_SPACE, VK_W,
};
use crate::ocr::{find_text_contains, ocr_screen};
use crate::stop_flag::should_stop;

use super::common::{
    buy_traps, clear_cache, start_game_with_difficulty, wait_for_game_end,
    IS_DEBUG, MOVE_VALUE,
};

/// 普通版难度标识
const DIFFICULTY: &str = "普通";
...
fn place_first_level_traps() -> Result<()> {
    ...
    println!("[place_traps] 进入放置模式 - 普通版");
    tap_key(VK_O);
    ...
    println!("[place_traps] 首关陷阱放置完成 - 普通版");
    tap_key(VK_O);
    thread::sleep(Duration::from_millis(500));
...
fn goto_safe_point() -> Result<()> {
    ...
    println!("[goto_safe_point] 移动到安全点 - 普通版");
